...
## 4.9 性能：为什么只有一个纯 Python 实现

纯 Python 递归约 20k 样本/秒。论文规模（$10^6$ 样本）实测：单点约 3 秒，
101 个点约 4.5 分钟——**对本仓库要产出的每一张图都够用**。

历史上这里还有一个 numba 后端（`mc/fast.py`：把递归化成显式栈，再用 `prange`
并行网格点），**已于 2026-08-14 删除**。理由：wavelab 的目的是**演示算法**而不是
做生产求解器，而 JIT 的约束（$\varphi,\psi$ 必须 `njit` 编译）加上维护第二套实现
的复杂度，超过了它省下的那点时间。代价是失去了 `test_backends_agree.py`——两套
独立实现互相验证的那道保险；正确性现在完全由闭式解 golden test 承担。需要时
可从 git 历史取回。

那个被删掉的技巧本身很漂亮，值得记住：因为 $H$ 是**所有节点权重的乘积**，根本
不需要真的建树再自底向上归并——维护一个待处理节点栈和一个运行乘积就够了，
递归于是化成了循环。
...
4. 代码即公式：`_sample` 每行对应推导的一步（4.8 节逐行走读）。
