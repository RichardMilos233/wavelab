...
| $t$ | MC 真值 $u(0.5,t)$ | θ-格式 | 与真值一致的有效数字 |
|---|---|---|---|
| 0.1 | 0.948 | **0.9479** ✓（格式本身实现正确！） | ~7 位 |
| 0.15 | — | 0.958 | ~2 位 |
| 0.2 | 0.990 | $O(10)$，量级已错 | 0 位 |
| 0.3 | 1.135 | $O(10^3)$，**连正负号都不确定** | 0 位 |
| 0.4 | 1.412 | $O(10^3)$，符号不确定 | 0 位 |

**为什么这里不写具体数字**：$N=101$ 时最高模的每步放大率 $g_*=1.5129$，于是
$10^{-16}$ 的舍入误差只需 $16\ln 10/\ln g_* \approx 89$ 步（$t\approx0.178$）就长到
$O(1)$——观测到的偏离时刻正是 $t\approx0.18$。换网格同样对得上：$N=51$ 时
$g_*=1.223$、预测 $t\approx0.37$（实测 $t=0.3$ 仍准、$t=0.4$ 变垃圾）；$N=201$ 时
$g_*=2.549$、预测 $t\approx0.08$（实测 $t=0.1$ 就已是垃圾）。既然输出**完全由舍入
模式决定**，它就随平台而变：在同一台机器上把 `np.linalg.solve` 换成等价的
`inv@G`／`lstsq`／改用差分模板算 Laplace 算子，$t=0.3$ 的值就在 1549…2782 之间跳、
$t=0.4$ 的**符号会翻**，而 $t=0.1$ 永远是 0.9479。

更甚者，失败其实有**第二阶段**：$t\gtrsim0.21$ 之后 Newton **根本不收敛**（199 步里
有 96 步打满 50 次迭代、残差仍有 $10^2$–$10^4$），那时的输出连"θ-格式的解"都算不
上。求解器现在会把这件事记进 `meta["newton_failed_steps"]` 并在首次发生时告警——
**不收敛也是数据**，和爆炸时刻一样。
